...
public class Main {
	...
	private static void showPage() {
		try {
			Desktop.getDesktop().browse(new File(TMP_DIR + "/javadoc/index.html").toURI());
	...
	private static void extractRequiredContent() {
		...
		Object[] toBeDeleted = new File(TMP_DIR + File.separator).list();
		toBeDeleted = contentRemove(toBeDeleted, "javadoc");
		toBeDeleted = contentRemove(toBeDeleted, "gettingstarted");
		toBeDeleted = contentRemove(toBeDeleted, "lockFile" + DoaEngine.VERSION);
		System.out.println(Arrays.toString(toBeDeleted));
		for (Object s : toBeDeleted) {
			delete(new File(TMP_DIR + File.separator + s));
		}
	}
	...
	private static Object[] contentRemove(Object[] array, Object element) {
		if (array.length > 0) {
			int index = -1;
